...
async function captureRetestScreenshots() {
  const chromePath = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
  const outDir = path.resolve(__dirname, '..', 'docs', 'screenshots', 'retest');
  ...
  // 1. Admin Dashboard
  await page.screenshot({ path: path.join(outDir, '01_retest_admin_dashboard.png') });
  console.log('Saved 01_retest_admin_dashboard.png');
  ...
  await page.screenshot({ path: path.join(outDir, '02_retest_part_master.png') });
  console.log('Saved 02_retest_part_master.png');
  ...
  await page.screenshot({ path: path.join(outDir, '03_retest_part_stock.png') });
  console.log('Saved 03_retest_part_stock.png');
  ...
  if (addBtn) {
    ...
    await page.screenshot({ path: path.join(outDir, '04_retest_packing_creation.png') });
    console.log('Saved 04_retest_packing_creation.png');
  ...
  await page.screenshot({ path: path.join(outDir, '05_retest_packing_barcode.png') });
  console.log('Saved 05_retest_packing_barcode.png');

  // 6. View Packing
  await page.goto('http://localhost:3000/view_packing', { waitUntil: 'networkidle2' });
  await page.waitForSelector('.data-table tbody tr');
  await new Promise((r) => setTimeout(r, 600));
  await page.screenshot({ path: path.join(outDir, '06_retest_view_packing.png') });
  console.log('Saved 06_retest_view_packing.png');
  ...
  await page.screenshot({ path: path.join(outDir, '07_retest_create_box.png') });
  console.log('Saved 07_retest_create_box.png');
  ...
  await page.screenshot({ path: path.join(outDir, '08_retest_empty_box_rejected.png') });
  console.log('Saved 08_retest_empty_box_rejected.png');
  ...
  await page.screenshot({ path: path.join(outDir, '09_retest_box_after_packing.png') });
  console.log('Saved 09_retest_box_after_packing.png');
  ...
  await page.screenshot({ path: path.join(outDir, '10_retest_box_after_lock.png') });
  console.log('Saved 10_retest_box_after_lock.png');
  ...
  await page.waitForSelector('.data-table tbody tr');
  await page.screenshot({ path: path.join(outDir, '11_retest_create_invoice.png') });
  console.log('Saved 11_retest_create_invoice.png');

  const testInvNum = `INV-RETEST-UI-${Date.now().toString().slice(-4)}`;
  ...
  await page.screenshot({ path: path.join(outDir, '12_retest_add_box_invoice.png') });
  console.log('Saved 12_retest_add_box_invoice.png');
  ...
  await page.screenshot({ path: path.join(outDir, '13_retest_invoice_matched.png') });
  console.log('Saved 13_retest_invoice_matched.png');
  ...
  // 14. Locked Invoice
  await page.screenshot({ path: path.join(outDir, '14_retest_invoice_locked.png') });
  console.log('Saved 14_retest_invoice_locked.png');
  ...
  await page.screenshot({ path: path.join(outDir, '15_retest_gate_verify.png') });
  console.log('Saved 15_retest_gate_verify.png');
  ...
  if (matchId) {
    ...
    await page.screenshot({ path: path.join(outDir, '16_retest_gate_scan.png') });
    console.log('Saved 16_retest_gate_scan.png');
    ...
    await page.screenshot({ path: path.join(outDir, '17_retest_gate_clearance.png') });
    console.log('Saved 17_retest_gate_clearance.png');
  }

  // 18. Gate-Out Report
  await page.goto('http://localhost:3000/gate_out_report', { waitUntil: 'networkidle2' });
  await new Promise((r) => setTimeout(r, 1000));
  await page.screenshot({ path: path.join(outDir, '18_retest_gate_out_report.png') });
  console.log('Saved 18_retest_gate_out_report.png');

  await db.end();
  await browser.close();
  console.log('\nALL 18 RETEST SCREENSHOTS CAPTURED SUCCESSFULLY in:', outDir);
}

captureRetestScreenshots().catch((err) => {
  console.error('[RETEST SCREENSHOT CAPTURE ERROR]:', err);
  process.exit(1);
});
